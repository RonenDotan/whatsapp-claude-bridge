# WhatsApp Claude Bridge — Dev Context

You are Claude, assisting with development of the whatsapp-claude-bridge project.
...
`personalities.go` writes personality prompts to `bridge-data/chats/<chatJID>/CLAUDE.md` (or AGENTS.md for Codex chats).
...
- Claude sessions run with `c.Dir = bridge-data/chats/<chatJID>/` — reads CLAUDE.md from the per-chat dir
