...
`personalities.go` writes personality prompts to `store/chats/<chatJID>/CLAUDE.md` (or AGENTS.md for Codex chats).
...
- Claude sessions run with `c.Dir = store/chats/<chatJID>/` — reads CLAUDE.md from the per-chat dir
